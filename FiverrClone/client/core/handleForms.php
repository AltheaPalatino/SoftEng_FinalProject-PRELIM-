...
if (isset($_POST['insertOfferBtn'])) {
	...
	$stmt = $pdo->prepare("SELECT * FROM offers WHERE user_id = ? AND proposal_id = ?");
	$stmt->execute([$user_id, $proposal_id]);


    //ERROR MESSAGE ONCE BECAUSE SUBMITTED ONCE PROPOSAL
	if ($stmt->rowCount() > 0) {
		echo "<script>alert('You have already submitted an offer to this proposal.'); window.history.back();</script>";
		exit;
	} else {
		$stmt = $pdo->prepare("INSERT INTO offers (user_id, description, proposal_id) VALUES (?, ?, ?)");
		$stmt->execute([$user_id, $description, $proposal_id]);

		echo "<script>alert('Offer submitted successfully!'); window.location.href='../project_offers_submitted.php';</script>";
		exit;
	}
}
